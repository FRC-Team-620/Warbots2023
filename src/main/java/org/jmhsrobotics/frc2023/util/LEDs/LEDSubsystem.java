...
public class LEDSubsystem extends SubsystemBase {
	...
	public static enum SectionManager {
		...
		/**
		 * Gets the total buffer length represented by the given LED sections in
		 * the SectionManager.
		 * 
		 * @param sections The LED sections from the SectionManager 
	...
	/**
	 * Copies the LED values stored in each LED section in the SectionManager
	 * to the main driver buffer (the buffer which is actually written to the
	 * LED strip/series of strips) in their appropriate locations.
	 * 
	 * <p> Note: This does not need to be called if you are using the 'sendSectionData'
	...
	/**
	 * Sends the LED data stored in the main driver buffer (the buffer which 
	 * is actually written to the LED strip/series of strips) to the LEDs.
	 * 
	 * <p> Note: Only one method which sends data to the LEDs (viz. 
	 * 'sendDriverBufferData' and 'sendSectionData') should be called per
	 * tick.
	 */
	 ...
	 * Copies the LED values stored in each LED section in the SectionManager
	 * to the main driver buffer (the buffer which is actually written to the
	 * LED strip/series of strips) in their appropriate locations, and then
	 * sends the LED data stored in the main driver buffer to the LEDs.
	 * 
	 * <p> Note: Only one method which sends data to the LEDs (viz. 
	 * 'sendDriverBufferData' and 'sendSectionData') should be called per
	 * tick.
	...
	/**
	 * Gets the main driver buffer (the buffer which is actually written to the
	 * LED strip/series of strips) so that all the LEDs can be written to at once.
	 * 
	 * <p> Note: Any animations that are created based on this buffer will act as
	 * though all of the LEDs on the robot constitute one LED section, and will
	 * loop through their entirety, which could be desirable.
	 * 
