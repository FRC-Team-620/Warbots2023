...
public class AutoBalance extends CommandBase {
	...
	// // tells whether it is on the charge station (charge station side sections
	// are
	// // 11 degrees,
	// // middle is 15 degrees when tipped)
	// //
	// // The robot should then record its position, to make sure it doesn't drive
	// back
	// // off the other side.
	// //
	...
	// // (there is a high threshold for level in this code as we don't want the
	// robot
	// // moving while it is in motion)
	// //
	// // If it reaches level, it should activate a PID to hold that position,
	// unless
	// // it stops being level.
	// //
	// // If it reaches a a limit, it should hold its position until it either needs
	// to
	// // move the other direction,
	// // or the robot is level, at which point it will also hold position.
	// //
	// // This command will never complete manually, as it needs to hold its
	// position.
	...
	// this.drivetrain.setCurvatureDrive(
	// hasBalanced ? AutoConstants.fineAdjustSpeed :
	// AutoConstants.balanceCreepSpeed, 0, false);
	// } else if (pitch > 0 && getRelativeDistance(chargeCenterPosition) <
	// moveLimit) {
	// this.drivetrain.setCurvatureDrive(
	// -1 * (hasBalanced ? AutoConstants.fineAdjustSpeed :
	// AutoConstants.balanceCreepSpeed), 0,
	// false);
	// } else {
	// if (!atLimit) {
	// atLimit = true;
	...
	// if (Math.abs(RobotContainer.getTelemetry().getPitch()) >
