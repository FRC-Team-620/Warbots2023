...
public class ScoringAuto extends SequentialCommandGroup {
	...
	// Constructor
	public ScoringAuto(Drivetrain drivetrain, ArmSubsystem armSubsystem, LEDSubsystem ledSubsystem, Enum startingPosition) {
		...
		if (this.startingPosition == startingPositions.LEFT) {
			...
			new AutoDriveDistance(drivetrain, 2), 
			new TurnDeltaAngle(drivetrain, -90), 
			new AutoDriveDistance(drivetrain, 0.2),
			new TurnDeltaAngle(drivetrain, 90), 
			new AutoDriveDistance(drivetrain, 2),
			new AlignPeg(drivetrain), 
			new CommandArm(armSubsystem, 0.2, 90), 
			new GrabberInOutAuto(false, 1),
			new AutoDriveDistance(drivetrain, -2),
			new TurnDeltaAngle(drivetrain, -90),
			new AutoDriveDistance(drivetrain, -0.2),
			new TurnDeltaAngle(drivetrain, 90),
